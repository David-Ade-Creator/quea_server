  ...
  if (!errors.isEmpty()) {
    ...
    User.findOne({
      ...
      return res.json({
        data: {
          user: user,
          accessToken: jwt.sign({ user }, config.CONFIRMATION_TOKEN_SECRET, {
            expiresIn: "1d",
          }),
        },
      });
      ...
      if (err) {
        ...
        user.save((err, user) => {
          if (err) {
            ...
            return res.json({
              message:
                "Your account has been successfully Activated, Login to continue.",
            });
          }
        });
      }
...
exports.googleLoginController = async (req, res) => {
  ...
  client
    .verifyIdToken({ idToken: idToken.tokenId, audience: config.GOOGLE_CLIENT })
    .then((response) => {
